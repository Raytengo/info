...
- `detail`：**加厚的說明段落（3–5 句，約 150–240 字）**，交代「方法／機制／中間過程」（它怎麼做到的、關鍵證據與數字）以及意義或反差，讓讀者讀完真的理解，而不只知道結論。段落結束後**另起一行**寫一句 bottom-line：`重點：<點出背後的意義或精髓，和 summary 錯開不重複>`。
...
3. **detail 要有實質，別為短而省。** 要交代方法、機制、中間過程與關鍵證據；使用者是為了讀懂內容而點進來，寧可多給脈絡，也不要只留結論。
...
語氣：客觀、精準，像跟懂行的朋友講重點，不要行銷腔。以繁體中文撰寫。
