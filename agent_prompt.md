...
- `summary`：一句話（45字內），講**最反直覺或最重要的那個發現**。
- `detail`：2–3 句（120字內），補「所以呢」——為什麼重要 / 方法巧在哪 / 限制或反差。
...
3. **detail 要補「so what」，不要重述。** 回答「為什麼重要 / 方法巧在哪 / 有什麼限制或反差」，而不是把 summary 的數字再講一次。
4. **抓張力。** 研究的精華常是反差——「能做 A 卻做不到 B」「反直覺的是…」「問題不在模型，在…」。把它寫出來。
